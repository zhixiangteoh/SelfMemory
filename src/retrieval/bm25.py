...
def parse_args():
    ...
    parser.add_argument(
        "--build_index", action="store_true", help="whether to train an index from scratch"
    )
    parser.add_argument(
        "--search_index", action="store_true", help="whether to search from a built index"
    )
...
def main(args):
    ...
    if args.build_index:
        ...
        for idx, (query, response) in tqdm.tqdm(
            enumerate(zip(queries, responses)), total=len(queries)
        ):
            action = {
                "_index": index,
                "_source": {
    ...
    if args.search_index:
        ...
        index = args.index_name
        query_body = {"query": {"match": {"query": None}}, "size": args.topk}
