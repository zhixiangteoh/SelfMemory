...
def move_to_device(maybe_tensor, device):
    ...
    if torch.is_tensor(maybe_tensor):
        ...
        return {key: move_to_device(value, device) for key, value in maybe_tensor.items()}
...
def analysis_DBS(hyps, refs, num_return_sequences=None):
    ...
    if not len(hyps) == len(refs):
        ...
        hyps = [
            hyps[idx : idx + num_return_sequences]
            for idx in range(0, len(hyps), num_return_sequences)
        ]
...
def evaluate_candidates(candidates, refs):
    ...
    for ref, candidate_ls in zip(refs, candidates):
        candidate_ls = [
            [candidate, get_rouge_score([candidate], [ref])[0]] for candidate in candidate_ls
        ]
